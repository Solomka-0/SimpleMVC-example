...
class ArticleController extends \ItForFree\SimpleMVC\mvc\Controller
{
    ...
    public function getCategoryAction() {
        ...
        $this->view->render('article/category.php'); // визуализация страницы
    }

    /**
     * Выводит сведения о подкатегории
     */
    public function getSubcategoryAction() {
        $subcategory_id =$_GET['id'];
        if (isset($subcategory_id)) {
            $this->subcategory = Config::getObject('core.subcategory.class')->getById($subcategory_id);
        }
        $this->view->addVar('subcategory', $this->subcategory); // передаём переменную по view
